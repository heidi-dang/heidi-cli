...
def parse_audit_decision(text: str) -> AuditDecision:
    ...
    if issues_match:
        ...
        decision.blocking_issues = [i.strip().lstrip("- ").strip() for i in issues_text.split("\n") if i.strip()]
    
    # Parse non_blocking
    non_blocking_match = re.search(r"non_blocking:\s*(.+?)(?:\n\w|\Z)", block, re.DOTALL)
    if non_blocking_match:
        non_blocking_text = non_blocking_match.group(1)
        decision.non_blocking = [i.strip().lstrip("- ").strip() for i in non_blocking_text.split("\n") if i.strip()]
    
    # Parse rerun_commands
    rerun_match = re.search(r"rerun_commands:\s*(.+?)(?:\n\w|\Z)", block, re.DOTALL)
    if rerun_match:
        rerun_text = rerun_match.group(1)
        decision.rerun_commands = [i.strip().lstrip("- ").strip() for i in rerun_text.split("\n") if i.strip()]
...
"""
            ...
            if self_audit_agent:
                self_audit_prompt = f"{self_audit_agent['prompt']}\n\nYour output:\n{run_res.output}"
