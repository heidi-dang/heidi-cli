...
export function currentPluginUpdateManifest(env: NodeJS.ProcessEnv = process.env): PluginUpdateManifest {
  return {
    ...
    summary: "Heidi v2.1 adds native zero-workspace bootstrap and an owner-full control profile while keeping authentication, secret isolation, and confirmed irreversible operations intact.",
    changes: [
    ...
    refresh_reason: "v2.1 adds a ChatGPT-facing MCP action, so the app action contract must be refreshed after deployment.",
