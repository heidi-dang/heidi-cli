...
#[test]
fn test_exact_m7_artifact_subject_digest_binding() {
    let tmp = tempdir().unwrap();
    let repo_root = tmp.path();
    let run_id = "run-subject-digest-1";
    ...
    let expected_sha256 = fdx::intelligence::runtime::sha256_bytes(&raw_bytes);
    ...
    assert_eq!(attestation.subject[0].digest.sha256, expected_sha256);
    assert_eq!(attestation.predicate.run.artifact_sha256, expected_sha256);
}
