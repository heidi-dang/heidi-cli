...
#[test]
fn test_atomic_and_idempotent_persistence() {
    let tmp = tempdir().unwrap();
    let repo_root = tmp.path();
    let run_id = "run-persistence-1";
    ...
    // First persistence -> creates file
    let (path1, sha1) = persist_attestation(repo_root, &attestation).unwrap();
    assert!(path1.exists());

    // Second persistence with identical content -> succeeds idempotently
    let (path2, sha2) = persist_attestation(repo_root, &attestation).unwrap();
    assert_eq!(path1, path2);
    assert_eq!(sha1, sha2);

    // Verify list_attestations finds it
    let list = list_attestations(repo_root).unwrap();
