...
class HuggingFaceIntegration:
    ...
    def __init__(self, token: Optional[str] = None):
        ...
        if heidi_home:
            self.cache_dir = Path(heidi_home) / "models" / "huggingface"
        else:
    ...
    async def search_models(
        self, query: str, task_filter: str = "text-generation", limit: int = 20
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for model_info in self.api.list_models(search=query, limit=limit, sort="downloads"):
    ...
    async def get_model_info(self, model_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            info = {
                ...
                or getattr(model_info.cardData, "description", "")
                ...
                "model_card": getattr(model_info, "cardData", {})
                if hasattr(model_info, "cardData")
                else {},
            ...
            if (
                ...
                raise ValueError(
                    f"❌ Model '{model_id}' not found on HuggingFace Hub. Please check the model name and try again."
                )
            elif (
                "401" in error_msg
                ...
                raise ValueError(
                    f"❌ Model '{model_id}' requires authentication or is private. Please check if you have access to this model."
                )
            elif "403" in error_msg or "forbidden" in error_msg:
                logger.error(f"Access forbidden for model: {model_id}")
                raise ValueError(
                    f"❌ Access to model '{model_id}' is forbidden. This may be a gated model requiring approval."
                )
            else:
                logger.error(f"Error getting model info for {model_id}: {e}")
                raise ValueError(
                    f"❌ Failed to get model info for '{model_id}'. Please check your internet connection and try again."
                )
    ...
    async def download_model(self, model_id: str, force_download: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Successfully downloaded {model_id}: {len(downloaded_files)} files, {metadata['size_gb']} GB"
            )
    ...
    async def auto_configure_model(self, model_id: str, model_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if any(
                indicator in model_info.get("tags", []) for indicator in function_calling_indicators
            ):
