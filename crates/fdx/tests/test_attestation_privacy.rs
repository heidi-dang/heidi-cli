...
#[test]
fn test_secrets_and_excerpts_excluded_from_attestation() {
    let tmp = tempdir().unwrap();
    let repo_root = tmp.path();
    let run_id = "run-privacy-1";
    ...
    let check = CheckExecutionResult {
        ...
        signal: None,
        duration_ms: 10,
        stdout_digest: Some("sha256:abcd".to_string()),
        stderr_digest: Some("sha256:ef01".to_string()),
        stdout_excerpt: Some("Bearer supersecret-token-do-not-leak".to_string()),
        stderr_excerpt: Some("AWS_SECRET_ACCESS_KEY=verysecret".to_string()),
        stdout_captured_bytes: 100,
        stderr_captured_bytes: 50,
    ...
    assert!(!canonical.contains("supersecret-token-do-not-leak"));
    assert!(!canonical.contains("AWS_SECRET_ACCESS_KEY"));
    assert!(!canonical.contains("stdout_excerpt"));
    assert!(!canonical.contains("stderr_excerpt"));
}
