...
learning_app = typer.Typer(
    help="Learning & Training management (reflect/export/curate/train-full/eval/promote/rollback)"
)
...
def _configure_model(model_path: str, model_id: Optional[str] = None):
    """Configure a local model in the suite config."""
    ...
    config.models.append({"id": model_id, "path": model_path, "backend": "transformers"})
    ...
    console.print(f"✓ Model {model_id} configured at {model_path}")
...
def setup(
    install: bool = typer.Option(
        False, "--install", "-i", help="Install dependencies and create virtual environment"
    ),
    model_path: Optional[str] = typer.Option(
        None, "--model", "-m", help="Path to local model directory"
    ),
    model_id: Optional[str] = typer.Option(None, "--model-id", help="Model ID for local model"),
):
    ...
    # =========================================================================
    # 1-CLICK INSTALL: Create venv and install dependencies
    # =========================================================================
    if install:
        console.print("[bold blue]🚀 Heidi CLI 1-Click Install[/bold blue]")
        console.print("Setting up virtual environment and installing dependencies...\n")
        ...
        if venv_path.exists():
            ...
            if venv_python.exists():
                console.print("[green]✓ Heidi CLI is already installed![/green]")
            ...
            console.print(f"Creating virtual environment at {venv_path}...")
            try:
                subprocess.run(
        ...
        console.print("Installing dependencies (this may take a few minutes)...")

        try:
            # Upgrade pip first
            subprocess.run(
                [str(pip_path), "install", "--upgrade", "pip"], check=True, capture_output=True
            )
            ...
            if result.returncode != 0:
                console.print(
                    f"[yellow]Warning: Some dependencies may have failed to install[/yellow]"
                )
                console.print(f"[dim]{result.stderr[:500]}[/dim]")
            else:
                console.print("✓ Dependencies installed successfully")
        except subprocess.CalledProcessError as e:
            console.print(f"[yellow]Warning: Installation had issues: {e}[/yellow]")

        # Set environment variable for analytics
        console.print("\n[bold]Configuring analytics storage...[/bold]")
        analytics_path = project_root / "state"
        console.print(f"✓ Analytics will be stored at: {analytics_path}")

        # Create activation instructions
        console.print("\n[bold green]🎉 Installation complete![/bold green]")
        console.print("\n[bold]Next steps:[/bold]")
        console.print(f"  1. Activate the environment:")
        console.print(f"     [cyan]source {venv_path}/bin/activate[/cyan]")
        console.print(f"\n  2. Run setup wizard to configure:")
        console.print(f"     [cyan]heidi setup[/cyan]")
        console.print(f"\n  3. Start the model server:")
        console.print(f"     [cyan]heidi model serve[/cyan]")
        console.print(f"\n[dim]Note: Add this to your shell profile for convenience:[/dim]")
        console.print(f"[dim]  alias heidi='source {venv_path}/bin/activate && heidi'[/dim]")

        # Optionally configure model if provided
        if model_path:
            _configure_model(model_path, model_id)
    ...
    if api_key:
        ...
        if (
            console.input("Would you like to enter your OpenCode API key? (y/n): ")
            .lower()
            .startswith("y")
        ):
    ...
    has_local_models = (
        console.input("Do you have local models to configure? (y/n): ").lower().startswith("y")
    )
...
def model_serve():
    ...
    cmd = [
        ...
        "-m",
        "uvicorn",
        "heidi_cli.model_host.server:app",
        "--host",
        config.host,
        "--port",
        str(config.port),
    ...
    try:
        with db.get_connection() as conn:
            ...
            for row in results:
                ...
                console.print(
                    f"[bold]Content:[/bold] {row['content'][:200]}{'...' if len(row['content']) > 200 else ''}"
                )
...
def hf_search(query: str, task: str = "text-generation", limit: int = 20):
    ...
    try:
        ...
        for i, model in enumerate(models[:limit], 1):
            ...
            console.print(
                f"   👤 {model['author']} | ⬇️ {model['downloads']:,} downloads | ❤️ {model['likes']}"
            )
            if model.get("tags"):
...
def hf_download(model_id: str, force: bool = False, add_to_config: bool = True):
    ...
    try:
        ...
        if add_to_config:
            ...
            config = asyncio.run(
                get_huggingface_integration().auto_configure_model(
            ...
            if config["id"] in existing_ids:
                console.print(
                    f"[yellow]Model {config['id']} already exists in configuration[/yellow]"
                )
                if (
                    not console.input("Overwrite existing configuration? (y/n): ")
                    .lower()
                    .startswith("y")
                ):
        ...
        console.print(
            f'  -d \'{{"model": "{config["id"] if add_to_config else model_id}", "messages": [{{"role": "user", "content": "Hello!"}}]}}\''
        )
...
def hf_compare(model_ids: List[str] = typer.Argument(..., help="Model IDs to compare")):
    ...
    console.print(
        f"[bold blue]📊 Comparing {len(model_ids)} models:[/bold blue] {', '.join(model_ids)}\n"
    )
    ...
    try:
        ...
        for model in models_info:
            tags = model.get("tags", [])
            langs = [
                tag for tag in tags if tag in ["english", "chinese", "french", "german", "spanish"]
            ]
        ...
        console.print(
            f"• Most Popular: {best_downloads.get('id')} ({best_downloads.get('downloads', 0):,} downloads)"
        )

        # Best for likes
        best_likes = max(models_info, key=lambda x: x.get("likes", 0))
        console.print(
            f"• Most Liked: {best_likes.get('id')} ({best_likes.get('likes', 0):,} likes)"
        )

        # Best for coding
        coding_models = [
            m for m in models_info if any(tag in m.get("tags", []) for tag in ["coding", "code"])
        ]
        if coding_models:
...
def hf_batch_download(
    model_ids: List[str] = typer.Argument(..., help="Model IDs to download"),
    force: bool = typer.Option(False, "--force", "-f", help="Force download even if model exists"),
    add_to_config: bool = typer.Option(
        True, "--add-to-config/--no-add-to-config", help="Add to Heidi configuration"
    ),
):
    ...
    def download_single_model(model_id):
        ...
        try:
            ...
            if add_to_config:
                config = asyncio.run(
                    hf.auto_configure_model(model_id, Path(metadata["local_path"]))
                )
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        future_to_model = {
            executor.submit(download_single_model, model_id): model_id for model_id in model_ids
        }
...
def hf_analytics(
    model_id: Optional[str] = typer.Argument(None, help="Model ID to analyze (optional)"),
    ...
    if model_id:
        ...
        if usage:
            ...
            if performance:
                ...
                perf_table.add_row(
                    "Throughput", f"{performance.throughput_requests_per_min:.1f} req/min"
                )
                perf_table.add_row("Error Rate", f"{performance.error_rate:.1%}")
                perf_table.add_row(
                    "Token Efficiency", f"{performance.token_efficiency:.1f} tokens/sec"
                )
            ...
            if export:
                ...
                export_file = (
                    Path.home() / ".heidi" / f"analytics_{model_id.replace('/', '_')}.json"
                )
...
def hf_remove(model_id: str):
    ...
    if (
        not console.input("Are you sure you want to remove this model? (y/n): ")
        .lower()
        .startswith("y")
    ):
